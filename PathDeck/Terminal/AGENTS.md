...
- 2026-06-18 S28 Quality Gate：`performKeyEquivalent` Ctrl+key 分支新增 IME composition 守卫——`markedText != nil` 时先调 `ghostty_surface_key_is_binding`，非 binding 交给 `super`（IME 处理），binding（如 Ctrl+C）仍发 libghostty。无 markedText 时行为不变。
