...
├── WorkspaceViewState.swift     per-window @Observable: mode / isTerminalVisible / activeTerminalID / terminalAnchorCwd / isCustomTitle / customTitle / isSidebarVisible / isPreviewPaneVisible（S36 起 per-window）/ isShortcutOverlayVisible（瞬态不持久化）
...
└── WorkspaceRootView.swift      per-window SwiftUI root：NavigationSplitView（columnVisibility ↔ viewState.isSidebarVisible）+ workspace content + Shortcut Overlay 挂载 + per-window NSEvent ⌘W/⌘T/浮窗 monitor（仅响应本 window keyWindow）
```
