...
- 依赖：Foundation、GRDB（SPM）、CryptoKit（仅 VersionStore SHA256）、SwiftUI（ChangeListView + DiffView）。
- 被依赖：`WorkspaceModel` 持有 `FSWatcher` + `ChangeStore` + `VersionStore`；`ContentView` 装载 `ChangeListView` + `DiffView`。
