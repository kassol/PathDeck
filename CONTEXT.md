...
**Path Link（路径链接）**:
终端输出中被识别为本地文件或目录引用的文本段，可带行列号（path:line[:col]），⌘Click 触发 Locate。相对形态只挂在该终端已知的 cwd 上；解析不出目标或目标不存在的文本段不是 Path Link。
_Avoid_: 超链接（保留给 URL）、文件链接

**Locate（定位）**:
把文件浏览器带到某个目标：目标是文件 → 定位到所在目录并选中，Preview Pane 随之展示；目标是目录 → 导航进入。Locate 不打开文件、不转移键盘焦点。
_Avoid_: 跳转；打开（保留给交由系统 app 的动作）
