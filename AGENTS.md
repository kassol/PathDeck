...
PathDeck/SidebarView.swift Sidebar（统一 Favorites 区块）+ PinnedFolders bookmark 持久化（首次启动 seed 默认五项，删空不恢复）
