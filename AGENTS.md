...
- 2026-06-16 **Pre-S22 稳定化**：Kill FinderSync Extension（FR-EXT-001/002）——FinderSync 只能承载右键菜单，无法提供主工作台能力，投入产出比不足；外部入口由 Services + URL Scheme + Open With + CLI helper 覆盖。.gitignore 修复 `design/shots/` 路径。10k 文件性能自动基线（cold-open/sort/filter）。reveal 测试修复 macOS `/var` firmlink 导致的 URL 不一致。xcodebuild 测试命令加 `-destination` 绕过 Developer Mode 未开时的 LaunchServices 报错。
- 2026-06-16 **S19–S21 质量加固**（审查链脆弱点收口）：三个独立切片。**S19** 终端 writeText 竞态——`PendingTextBuffer`（FIFO + 条数/字节上限）+ `onSurfaceFailed`（surface 创建失败立即丢弃并经 `onPendingDropped` 上报，不再盲等 5s）+ 超时 token 化可取消 + `handleSurfaceClose` 遍历全部已退出 surface（`exitedSessionIDs` 纯函数）。**S20** 变化归因重构——`TerminalAttribution.attribute` 纯函数按 cwd 前缀匹配（最深优先、同深度取最近活跃、不匹配 → nil），`FSWatcher` 事件入队时刻捕获终端活跃快照（coalesce 不覆盖），`recordBatch` 改 per-event 归因，删除「贴当前选中 tab」的 `activeTerminalSessionID`。**S21** 版本 baseline——进目录后台预录既存文本文件基线（16MB 预算）修复首次外部修改拿不到真 before，`FileRestore`/`selectBaseline` 纯函数化 + 外部脏三态文案。三块各自单 commit、全量单测绿（156→165→171）。详见 `docs/plans/2026-06-16-s19-s21-quality-hardening.md`。同批含两轮 review 加固（FSWatcher flush 事件绑定 origin 目录、watcher 状态收敛 `queue.sync`、restore 安全落盘、baseline rescan 去重）+ FSWatcher origin-directory 回归测试 + 独立修复终端 `exit` 自动关闭（`GHOSTTY_ACTION_SHOW_CHILD_EXITED` action；S13 假设的 `wait_after_command` 在 embedder 模型下不负责退出关闭）。child-exit 行为依赖真实子进程退出、单测覆盖不到，其 GUI 验证 + 含 review/回归测试的全量单测复跑待在 Xcode 确认。
- 2026-06-16 **S18 系统入口**（M5 Phase 2）：新增 `AppRouter`（@Observable 单例，三类外部入口归一为一次性令牌路由）+ `URLSchemeHandler`（`pathdeck://open|reveal|terminal?path=` 查询参数式解析 + 目录/存在性安全校验，非法静默丢弃）+ `ServicesProvider`（Finder Services @objc handler ×4，`NSPasteboard` → `AppRouter.Route`）+ `AppDelegate`（`application(_:open:)` 统一 URL Scheme 与 Open With file URL，不挂 `.onOpenURL` 规避 WindowGroup 复制窗口；`applicationDidFinishLaunching` 注册 `servicesProvider`）+ `WorkspaceModel.reveal(_:)`（`[URL]` 选择集，导航首项父目录 + 同父目录多选高亮、跨父目录项忽略，兑现 Open Selection 多选语义）+ `ContentView` 经 onAppear/onChange 消费 `router.pending`（冷启动与运行时唤起双时序覆盖，consume 幂等）。`Info.plist` 独立化（`CFBundleURLTypes`/`NSServices`×4/`CFBundleDocumentTypes`/`LSMultipleInstancesProhibited`），`GENERATE_INFOPLIST_FILE=YES` + `INFOPLIST_FILE` 片段合并（plutil 验证自定义键与自动生成键共存）；pbxproj `INFOPLIST_FILE`×2 + `Info.plist` 加入 synchronized group `membershipExceptions`。CLI helper 留 S22，FinderSync Kill（见 Pre-S22 条目）。review 后修订：① Open Selection 多选兑现「同父目录多选高亮」（`Route.reveal([URL])`，跨父目录项忽略；表格选择信号 `scrollToURL: URL?` 升级为 `revealSelection: [URL]?`，`FileTableView` 用 `IndexSet` 一次性选中全部行+滚动首项，修复多选只落单行）；② URL Scheme 的 terminal route 标记 `requireConfirmation=true`，开 shell 前 `NSAlert` 用户确认（PRD 1214「无权限路径需用户确认」；Finder Services「Open Terminal Here」为用户主动操作置 false 不确认）。149 个单测通过（新增 `AppRouter`/`URLSchemeHandler`/`ServicesProvider`/`reveal` 用例 + 多选/确认标记，临时目录隔离）。
- 2026-06-15 **S17 窗口状态恢复 + writeText 竞态修复 + 性能基线**（M5 Phase 1）：`GhosttyTerminalEngine` 引入 pending text buffer + `onSurfaceReady` 回调（修复 `sendPathToTerminal` 在新建 tab 时文本丢失竞态）；`ContentView` 全面状态持久化（底部面板展开/高度、预览面板、activeTab → `UserDefaults`，terminal tabs → JSON 序列化恢复占位）；`WorkspaceModel` 排序列/方向/隐藏文件持久化（`UserDefaults` didSet 写回）；`StatePersistenceModifier` 拆分 onChange 链降低类型检查复杂度；性能测试（1k 文件 reload/sort/filter 自动基线，默认随套件跑）；`WorkspaceModel` 注入 `defaults: UserDefaults` 参数隔离测试。126 个单测通过（+9）。
