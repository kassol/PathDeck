...
- 2026-07-17 **S39 可测缝下沉**：FR-BRIDGE-003 人工走查项能程序化的全部下沉为单测——像素→格子换算纯函数（`GhosttyGridCellTests`）、悬停时序状态机 `PathLinkHoverTracker`（`PathLinkHoverTrackerTests`）、真窗口 Locate 行为（`LocateBehaviorTests`：文件选中不夺焦/目录进入/reveal 默认夺焦回归）。测试坑记录：MainActor 测试体等 `DispatchQueue.main.async` 结果必须 await 让出，`RunLoop.run` drain 不了 main queue（Terminal/AGENTS.md「验证」节）。全量 380。
- 2026-07-16 **S39 FR-BRIDGE-003 #7 ⌘悬停反馈**：⌘+悬停命中 Path Link → pointingHand + 浮层显示解析后绝对路径（补偿终端渲染层无法画下划线的可发现性缺口）；松 ⌘/移出/失焦/滚动即恢复。复用 ⌘Click 命中管线，`hoverCell` 格子级缓存防快速扫过卡顿；`hoverOverlayOrigin` clamp 纯函数 4 例单测，全量 361。FR-BRIDGE-003 四票（#4–#7）全部落地。
- 2026-07-16 **S39 FR-BRIDGE-003 #6 识别语法全集与 cwd 语义**：检测器扩展到 PRD 全集——相对路径仅挂 OSC 7 上报 cwd（`GhosttySurfaceView.reportedCwd`，未知不触发）、`~` 展开、`path:line[:col]`（剥离查存在，行列号进 `PathLink.line/column` 保留不消费，字面含冒号文件名优先）、引号包裹含空格路径（引号区候选优先、落空回退 token）、`file://` 按本地路径 Locate（含 OPEN_URL 的 file:// 改道 `registerFileURLHandler`→engine→Locate，堵住 OSC 8 file:// 被系统打开违反 ADR-0003 的口子）；Windows 反斜杠明确不做（负例）。`PathLinkDetectorTests` 15→36 例，全量 357。
