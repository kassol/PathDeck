...
- 2026-06-18 **S28 Quality Gate**：IME composition 活跃时 `performKeyEquivalent` Ctrl+key 增加 `is_binding` 前置检查（非 binding 交给 IME，binding 如 Ctrl+C 仍发 libghostty）。FSWatcher 补 3 个边界测试（re-watch 重启 / 动态 expanded dirs / 移除 expanded dir 停止匹配）。PRD 全量同步 D1 Kill Change Journal 决策（§9.5 子 FR + §11.1 + §12.3-12.5 + §14 + §15 + M0 + §20 文案）。10k 性能基线阈值记录（CLI 执行受 LaunchServices 限制，实测值 pending Xcode GUI 跑通）。
